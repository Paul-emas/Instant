...
const ContactCard = ({ top, light, iconLight }) => {
  return (
    <div className={`${top ? 'top-24 2xl:top-40' : null} relative hidden items-center lg:flex`}>
      {!iconLight ? (
      ...
      <div className="ml-4">
        <p className={`${light ? 'text-gray-400' : 'text-font-darker'} relative top-1 text-xs`}>
          Need help?
        </p>
        <p className={`${light ? 'text-black' : 'text-font-darker'} text-lg font-semibold `}>
          090-8233-3376
        </p>
      </div>
    </div>
